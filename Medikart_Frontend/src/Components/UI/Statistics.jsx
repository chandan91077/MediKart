  ...
  useEffect(() => {
    ...
    const fetchOrderStats = async () => {
      try {
        const response = await axios.post(
          "https://api.prabhatanvik.shop/order/salesStats",
          { month: currentMonth },
    ...
    const fetchChanges = async () => {
      try {
        ...
        const responseCurrentMonth = await axios.post(
          "https://api.prabhatanvik.shop/order/salesStats",
          { month: currentMonth },
          { withCredentials: true }
        );
        const responsePreviousMonth = await axios.post(
          "https://api.prabhatanvik.shop/order/salesStats",
          { month: previousMonth },
          { withCredentials: true }
        );
        ...
        if (
          responseCurrentMonth.data !== "no sales found for this month" &&
          responsePreviousMonth.data !== "no sales found for this month"
        ) {
          ...
          const totalItemsChange =
            currentMonthData.totalItems - previousMonthData.totalItems;
          ...
          if (previousMonthData.totalOrders !== 0) {
            totalOrdersPercentageChange =
              (totalOrdersChange / previousMonthData.totalOrders) * 100;
          } else {
            totalOrdersPercentageChange = 100;
          }
          if (previousMonthData.totalItems !== 0) {
            totalItemsPercentageChange =
              (totalItemsChange / previousMonthData.totalItems) * 100;
          } else {
            totalItemsPercentageChange = 100;
          }
          if (previousMonthData.totalRevenue !== 0) {
            totalRevenuePercentageChange =
              (totalRevenueChange / previousMonthData.totalRevenue) * 100;
          } else {
