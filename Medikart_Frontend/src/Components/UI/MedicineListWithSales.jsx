...
export function MedicineListwithSales() {
  ...
  const deleteMedicine = async (id) => {
    ...
    try {
      await axios.delete(
        `https://api.prabhatanvik.shop/admin/availableMedicines/${id}`
      );
  ...
  const updateMedicine = async (updatedMedicine) => {
    try {
      ...
      await axios.put(
        `https://api.prabhatanvik.shop/admin/availableMedicines/${updatedMedicine._id}`,
        updatedMedicine
      );
  ...
  return (
    <>
      <Navigation pagetitle={"Stock Management"} />
      <SidebarDR mb={3} />
