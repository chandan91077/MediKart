    ...
    if (item.Onboard == false) {
      for (let i = 0; i < user.Prescriptions.length; i++) {
        for (let j = 0; j < user.Prescriptions[i].Medicine.length; j++) {
          if (
            user.Prescriptions[i].Medicine[j].Onboard == false &&
            user.Prescriptions[i].Medicine[j].Quantity == item.quantity
          ) {
    ...
    const pipeline = [
      {
        $match: {
          date_added: {
            ...
            $lt: new Date(
              new Date(chosenMonth).setMonth(chosenMonth.getMonth() + 1)
            ), // End of chosen month
          },
...
module.exports.MonthlyChanges = async (req, res) => {
  try {
    ...
    const previousMonthNum =
      currentMonthNum - 1 === 0 ? 12 : currentMonthNum - 1; // Get the previous month's number

    const previousMonth = Object.keys(monthsMap).find(
      (key) => monthsMap[key] === previousMonthNum
    ...
    const ordersIncrease =
      currentMonthData[0].totalOrders -
      (previousMonthData[0]?.totalOrders || 0);
    const itemsIncrease =
      currentMonthData[0].totalItems - (previousMonthData[0]?.totalItems || 0);
    const revenueIncrease =
      currentMonthData[0].totalRevenue -
      (previousMonthData[0]?.totalRevenue || 0);
