  ...
  try {
    ...
    if (user) {
      ...
      const specificRoom = user.chatRooms.find(
        (room) => room.doctorUsername === selectedDoctor
      );
...
const getMedAndRelatedProducts = async (req, res) => {
  ...
  try {
    ...
    const relatedMed = await medModel.find({
      Details: med.Details,
      Name: { $ne: med.Name },
    });
...
const userPrescription = async (req, res) => {
  ...
  const user = await userModel.findById(userid);
  if (!user || !user.Prescriptions) {
    return res.status(404).json({ message: "User or prescriptions not found" });
  }
    ...
    if (user.Prescriptions[i].id == PrescriptionId) {
      for (let j = 0; j < user.Prescriptions[i].Medicine.length; j++) {
        if (user.Prescriptions[i].Status != "Filled") {
          const Medicene = await medModel.find({
            Name: user.Prescriptions[i].Medicine[j].MedicineName,
          });
