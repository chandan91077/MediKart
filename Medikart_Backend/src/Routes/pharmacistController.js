...
const editMedicineResults = async (req, res) => {
  try {
    ...
    if (existingMedicine) {
      ...
      if (newPrice) existingMedicine.Price = newPrice;
      if (newDetails) existingMedicine.Details = newDetails;
...
const join = async (req, res) => {
  try {
    ...
    const existingChatRoom = patient.chatRooms.find(
      (room) =>
        room.doctorUsername === doctorUsername && room.username === username
    );
...
const sendMessage = async (req, res, socket) => {
  ...
  try {
    ...
    if (user) {
      ...
      const specificRoom = user.chatRooms.find(
        (room) => room.doctorUsername === doctorUsername
      );
...
const joinDoctorClin = async (req, res) => {
  try {
    ...
    const existingChatRoom = pharmacistt.chatRooms.find(
      (room) =>
        room.doctorUsername === docClinUsername &&
        room.pharmacistUsername === doctorUsername
    );
...
const sendMessage2 = async (req, res, socket) => {
  ...
  try {
    ...
    if (pharmacist) {
      // Find the specific room within the chatRooms array
      const specificRoom = pharmacist.chatRooms.find(
        (room) => room.doctorUsername === docClinUsername
      );
